...
```{r}
ggplot(hcris.dat.merged, aes(x=expand_ever, y=baddebtoperating)) + 
...
# Plotting the top five states that show the greatest decrease in bad debt over time
```{r}
```


# Estimating the effect of Medicaid expansion on bad debt using a standard DD regression estimator, again focusing only on hospitals/states that expanded in 2014 versus those that never expanded
```{r}
...
# Include hospital and year fixed effects in your estimates. 
```{r}
summary(felm(baddebtoperating ~ treat | factor (provider_number) + factor(year), data = hcris.dat.2014.merged))

```

# Repeat the analysis in question 3 but include all states (even those that expanded after 2014).
```{r}
